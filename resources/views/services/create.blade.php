...
<div class="px-4 sm:px-0">
    <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-6">Crear Servicio</h1>

    <form action="{{ route('services.store') }}" method="POST" class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div>
                <label for="slug" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Slug (opcional, se genera automáticamente)</label>
                <input type="text" name="slug" id="slug" value="{{ old('slug') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white font-mono">
            </div>

            <div>
                <label for="endpoint" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Endpoint (opcional)</label>
                <input type="url" name="endpoint" id="endpoint" value="{{ old('endpoint') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            </div>
                ...
                <div class="mt-2 flex gap-4">
                    <label class="inline-flex items-center">
                        <input type="radio" name="service_type" value="web"
                               {{ old('service_type', 'web') === 'web' ? 'checked' : '' }}
            ...
            <div id="ssh-fields" class="{{ old('service_type') === 'ssh' ? '' : 'hidden' }} grid grid-cols-1 gap-6 sm:grid-cols-2 border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-gray-50 dark:bg-gray-900">
                ...
                <div>
                    ...
                    <input type="text" name="ssh_host" id="ssh_host" value="{{ old('ssh_host') }}"
                ...
                <div>
                    ...
                    <input type="text" name="ssh_user" id="ssh_user" value="{{ old('ssh_user', 'admin') }}"
